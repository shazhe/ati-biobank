...
% Find all variables and their visits (using mergeV code)
varsM1 = []; % The last available visit
varsM2 = []; % The average from available visits
for ii = 1:n_vars
    varstemp = dirty(:, all_names == names(ii));

    ncols = repmat(1:size(varstemp, 2), size(varstemp, 1), 1);   
    namat = ~isnan(varstemp);

    lastVisit = max(namat .* ncols, [], 2);
    for jj = 1:length(lastVisit)

        % Store only last visit
        if lastVisit(jj) < 1
            varsM1(jj, ii) = NaN;
        else
            varsM1(jj, ii) = varstemp(jj, lastVisit(jj));
        end

        % Store all visits
        data(:, ii, jj) = varstemp(:, lastVisit(jj));
    end
    varsM2(:, i) = nanmean(varstemp, 2);
end
fprintf('--- Done! ---\n')
